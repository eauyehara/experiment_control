...
# -*- coding: utf-8 -*-
"""
LI curve measurement script
1. Power meter (HP 8153A)
2. Source meter (Keysight B2902A)
"""
...
class Window(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.motor_steps = 0
            ...
            # wavelength sweep related variables
        self.wavelength_start = Q_(550.0, 'nm')
        self.wavelength_stop = Q_(554.0, 'nm')
        self.wavelength_step = Q_(5.0, 'nm')
        self.exp_N = 500

            # LI curve related variables
        self.li_start = Q_(0.0, 'A')
        self.li_stop = Q_(5e-3, 'A')
        self.li_step = Q_(1e-3, 'A')
    ...
    def initialize_gui(self):
        ...
        if self.smu is not None and self.pm is not None:
            measLIAction = QtGui.QAction("Measure &L-I curve", self)
            measLIAction.setShortcut("Ctrl+L")
            measLIAction.setStatusTip('Place over device to measure power')
            measLIAction.triggered.connect(self.exp_li)
            experimentMenu.addAction(measLIAction)
        ...
        if self.smu is not None:
            ...
            col = 0
            self.label_li_start=QtGui.QLabel('LI start (A)')
            self.layout.addWidget(self.label_li_start, row, col,  1,1)
            col = col+1
            self.edit_li_start = QtGui.QLineEdit('{}'.format(self.li_start.magnitude))
            self.layout.addWidget(self.edit_li_start, row, col,  1,1)
            col = col+1

            self.label_li_stop=QtGui.QLabel('LI stop (A)')
            self.layout.addWidget(self.label_li_stop, row, col,  1,1)
            col = col+1
            self.edit_li_stop = QtGui.QLineEdit('{}'.format(self.li_stop.magnitude))
            self.layout.addWidget(self.edit_li_stop, row, col,  1,1)
            col = col+1

            self.label_li_step=QtGui.QLabel('LI step (A)')
            self.layout.addWidget(self.label_li_step, row, col,  1,1)
            col = col+1
            self.edit_li_step = QtGui.QLineEdit('{}'.format(self.li_step.magnitude))
            self.layout.addWidget(self.edit_li_step, row, col,  1,1)
            row = row+1
        ...
        self.layout.addWidget(self.p_spec, 0, 6, 10, 10)
        ...
        self.layout.addWidget(self.p_power, 10, 6, 10, 10)
        ...
        self.layout.addWidget(self.p_current, 20, 6, 10,10)
    ...
    def exp_li(self):
        ...
        if len(measDescription)>0:
            ...
            #  Load measurement parameters
            li_current = Q_(float(self.edit_li_start.text()), 'A')
            li_stop = Q_(float(self.edit_li_stop.text()), 'A')
            li_step = Q_(float(self.edit_li_step.text()), 'A')

            # fix step errors
            if li_current < li_stop and li_step < Q_(0.0, 'A'):
                li_step = -li_step
            elif li_current > li_stop and li_step > Q_(0.0, 'A'):
                li_step = -li_step

            data_out = []
            while (li_current <= li_stop and li_step>Q_(0.0, 'A')) or (li_current >= li_stop and li_step<Q_(0.0, 'A')):
            ...
            self.statusBar().showMessage('Canceled L-I curve', 1000)
